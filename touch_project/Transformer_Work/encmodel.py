...
parser.add_argument(
    "--d_model", help="embedding dimension for model", type=int
)
...
parser.add_argument(
    "--eval_iters",
    help="Every eval_iters steps, loss will be validated",
    type=int,
)
...
def plot_lr(opt):
    sched1 = lr_scheduler.StepLR(
        optimizer=opt, step_size=args.max_iters // 10, gamma=0.6
    )
    sched2 = lr_scheduler.CosineAnnealingWarmRestarts(
        optimizer=opt, T_0=args.max_iters // 2, eta_min=1e-6
    )
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value):
        ...
        query, key, value = [
            linear(x).view(-1, self.h, self.d_k).transpose(1, 2)
...
class EncModel(nn.Module):
    def __init__(
        ...
        self.decoder = Decoder(
            DecoderLayer(d_model, c(attn), c(ff), dropout), N, ln_gain_mult=1.0
        )
...
optimizer = torch.optim.Adam(
    model.parameters(), lr=args.lr, betas=(0.9, 0.98), eps=1e-8
)
scheduler = lr_scheduler.StepLR(
    optimizer=optimizer, step_size=args.max_iters // 10, gamma=0.6
)
...
for iter in tqdm(range(1, args.max_iters + 1)):
    ...
    if iter % args.eval_iters == 0:
        ...
        if np.mean(valids) < best_validation_loss:
            ...
            torch.save(model, args.save_dir + "enc_model.pt")
        model.train()
end = timer()
